...
class EquipmentApp:
    def __init__(self, master):
        ...
        # Category Button (in the left frame)
        self.category_button = tk.Button(self.button_frame, text="Category", command=self.open_category_window,
                                         bg=self.button_color, fg=self.button_text_color, font=self.font, bd=0, padx=35, pady=5)
        self.category_button.pack(side="top", fill="x", pady=5)

        # Exit Button (in the left frame, aligned with the bottom buttons)
    ...
    def open_edit_equipment_window(self):
        ...
        def on_edit_confirm():
            if validate_entries():
                ...
                condition = condition_combobox.get()
                available_to_use = availability_combobox.get() == "Yes"
                self.current_category.get_equipment()[item_index] = Equipment(name, quantity, condition, available_to_use)
                self.refresh_equipment_list()
